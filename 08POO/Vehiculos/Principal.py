...
gestionBicis = ListaBicicletas()
gestionCoches = ListaCoches()
...
def crearCoche():
    ...
    mensaje=gestionCoches.anadirCoches(coche)
    if mensaje == True:
...
def crearBici():
    ...
    mensaje=gestionBicis.anadirBicis(bici)
    if mensaje == True:
...
while opcion !=8:
    ...
    if opcion == 1:
        ...
        coche = gestionCoches.buscarCoche(id)
        if (coche == None):
            print("El coche no existe")
        else:
            gestionCoches.eliminarCoche(id)
    elif opcion == 4:
        id = input("Dime el ID de la bicicleta a borrar: ")
        bici = gestionBicis.buscarBici(id)
        print(bici)
        if (bici == None):
            print("La bici no existe")
        else:
            gestionBicis.eliminarBici(id)
    elif opcion == 5:
        ...
        gestionBicis.mostrarBicis()
        print()
        gestionCoches.mostrarCoches()
        print()
